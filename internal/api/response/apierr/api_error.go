...
var (
	...
	OperationFailed    = response.NewJsonResError(200_00_004, "操作失败")               // 操作失败
	CreateFailed       = response.NewJsonResError(500_00_005, "创建失败")               // 创建失败
	UpdateFailed       = response.NewJsonResError(500_00_006, "更新失败")               // 更新失败
	SaveFailed         = response.NewJsonResError(500_00_007, "保存失败")               // 保存失败
	DeleteFailed       = response.NewJsonResError(500_00_008, "删除失败")               // 删除失败
	FetchFailed        = response.NewJsonResError(500_00_009, "查询失败")               // 查询失败
	AuthInvalidUserId  = response.NewJsonResError(401_00_010, "鉴权失败,无效的userId")     // 鉴权失败,无效的userId
	AuthInvalidToken   = response.NewJsonResError(401_00_011, "鉴权失败,无效的token")      // 鉴权失败,无效的token
	AuthExpired        = response.NewJsonResError(401_00_012, "鉴权失败,授权已过期")         // 鉴权失败,授权已过期
	AuthUserIdMismatch = response.NewJsonResError(401_00_013, "鉴权失败,userId与其凭证不匹配") // 鉴权失败,userId与其凭证不匹配
	AuthClaimsExpired  = response.NewJsonResError(401_00_014, "鉴权失败,登录已失效")         // 鉴权失败,登录已失效
